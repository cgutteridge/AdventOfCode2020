...
sub parse {
	...
	my $exp = parse_mult_expression( $chars );
	if( @$chars ) {
...
#EXP = ADD_EXP ( '*' ADD_EXP )*
sub parse_mult_expression {
	my( $chars ) = @_;

	my $exp = parse_add_expression($chars);;

	while( @$chars && $chars->[0] eq "*" ) {
		my $operator = shift @$chars;
		my $tail_value = parse_add_expression( $chars );
...
sub parse_value {
	...
	if( $chars->[0] eq "(" ) {
		shift @$chars; # consume "("
		$exp = parse_mult_expression( $chars );
		if( !@$chars || $chars->[0] ne ")" ) {
			die "expected )";
		}
		shift @$chars; # consume ")"
